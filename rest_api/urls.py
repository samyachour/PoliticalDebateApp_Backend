...
urlpatterns = [
    ...
    path('auth/register/', RegisterUsersView.as_view(), name=auth_register_name),
    path('auth/change-password/', ChangePasswordView.as_view(), name=auth_change_password_name),
    path('{0}/change-email/', ChangeEmailView.as_view(), name=auth_change_email_name),
    path('{0}/token/obtain/', TokenObtainPairView.as_view(), name=auth_token_obtain_name), # Login
    path('{0}/token/refresh/', TokenRefreshView.as_view(), name=auth_token_refresh_name),
